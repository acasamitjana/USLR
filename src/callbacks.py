...
class LRDecay(Callback):
    ...
    def on_train_init(self, model, **kwargs):
        ...
        if init_epoch >= self.n_iter_start:
            updated_lr = (1 - (init_epoch - self.n_iter_start)/(self.n_iter_finish-self.n_iter_start)) * self.lr_init
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = updated_lr

    def on_epoch_fi(self, logs_dict, model, epoch, **kwargs):
        if epoch >= self.n_iter_start:
            updated_lr = (1 - (epoch - self.n_iter_start)/(self.n_iter_finish-self.n_iter_start)) * self.lr_init
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = updated_lr
